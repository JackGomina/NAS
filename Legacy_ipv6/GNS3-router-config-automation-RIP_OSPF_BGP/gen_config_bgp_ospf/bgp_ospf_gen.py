...
def generate_bgp_configs(topology_file, output_dir="configs", options=None):
    ...
    # Build a per-router set of interfaces that belong to provider-provider links.
    mpls_core_ifaces = {}
    for link in links:
        a = link.get("a")
        b = link.get("b")
        ...
        if role_a in provider_roles and role_b in provider_roles:
            mpls_core_ifaces.setdefault(a, set()).add(a_iface)
            mpls_core_ifaces.setdefault(b, set()).add(b_iface)
    ...
    for router in routers:
        ...
        for iface in router.get("interfaces", []):
            ...
            iface_copy["ospf_enabled"] = enable_ospf
            iface_copy["mpls_enabled"] = iface.get("name") in mpls_core_ifaces.get(name, set())
